...
import 'package:career_coach/presentation/screen/section_resume/certificate/cubit/certificate_cubit.dart';
...
class CertificateScreen extends StatefulWidget {
  const CertificateScreen({super.key, required this.userResumeId});
...
class _CertificateScreenState extends State<CertificateScreen> {
  ...
  final List<TextEditingController> _listCertificatedAtController = [];
  final List<TextEditingController> _listCertificateLinkController = [];

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => CertificateCubit()..init(userResumeId: widget.userResumeId),
      child: Scaffold(
        appBar: AppBar(
          leading: InkWell(
            onTap: () {
  ...
  Widget _listView() {
    return BlocConsumer<CertificateCubit, CertificateState>(
      listener: (context, state) {
        ...
        _listCertificatedAtController.clear();
        _listCertificateLinkController.clear();
        for (var it in state.listCertificate) {
          _listNameController.add(TextEditingController(text: it.name));
          _listCertificatedAtController.add(TextEditingController(text: it.certificatedAt));
          _listCertificateLinkController.add(TextEditingController(text: it.certificateLink));
        }
      },
      builder: (context, state) {
        return ListView.separated(
          itemCount: state.listCertificate.length,
            ...
            return Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 8.0),
                  BaseContentDate(
                    controller: _listCertificatedAtController[index],
                    isRequired: false,
                    title: context.language.time,
                    onTap: () async {
                      DateTime? currentDate;
                      if( _listCertificatedAtController[index].text.isNotEmpty) {
                        currentDate = DateFormat('yyyy-MM-dd').parse(_listCertificatedAtController[index].text);
                      }
                      ...
                      if (pickedDate != null) {
                        _listCertificatedAtController[index].text = DateFormat('yyyy-MM-dd').format(pickedDate);
                      }
                    },
                  ),
                  SizedBox(height: 8.0),
                  BaseContent(
                    controller: _listCertificateLinkController[index],
                    isRequired: false,
