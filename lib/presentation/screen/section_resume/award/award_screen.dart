...
import 'package:career_coach/presentation/screen/section_resume/award/cubit/award_cubit.dart';
...
class AwardScreen extends StatefulWidget {
  const AwardScreen({super.key, required this.userResumeId});
...
class _AwardScreenState extends State<AwardScreen> {
  ...
  final List<TextEditingController> _listAwardedAtController = [];

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => AwardCubit()..init(userResumeId: widget.userResumeId),
      child: Scaffold(
        appBar: AppBar(
          leading: InkWell(
            onTap: () {
  ...
  Widget _listView() {
    return BlocConsumer<AwardCubit, AwardState>(
      listener: (context, state) {
        ...
        _listAwardedAtController.clear();
        for (var it in state.listAward) {
          _listNameController.add(TextEditingController(text: it.name));
          _listAwardedAtController.add(TextEditingController(text: it.awardedAt));
        }
      },
      builder: (context, state) {
        return ListView.separated(
          itemCount: state.listAward.length,
                ...
                children: [
                  Row(
                    children: [
                      ...
                      if (state.listAward[index].displayOrder != state.listAward.length - 1) ...[
                        _buttonChange(type: 1, onChange: () {
                          saveTemp(state.listAward);
                          context.read<AwardCubit>().changeAward(index, index + 1);
                        }),
                        SizedBox(width: 16),
                      ],
                      if (state.listAward[index].displayOrder != 0) ...[
                      ...
                      _buttonChange(type: 0, onChange: () {
                        saveTemp(state.listAward);
                        context.read<AwardCubit>().deleteAward(index);
                      }),
                    ],
                  ),
                  BaseContent(
                  ...
                  SizedBox(height: 8.0),
                  BaseContentDate(
                    controller: _listAwardedAtController[index],
                    isRequired: false,
                    title: context.language.time,
                    onTap: () async {
                      DateTime? currentDate;
                      if( _listAwardedAtController[index].text.isNotEmpty) {
                        currentDate = DateFormat('yyyy-MM-dd').parse(_listAwardedAtController[index].text);
                      }
                      ...
                      if (pickedDate != null) {
                        _listAwardedAtController[index].text = DateFormat('yyyy-MM-dd').format(pickedDate);
                      }
                    },
                  ),
                ],
  ...
  Widget _buttonSave() {
    return BlocBuilder<AwardCubit, AwardState>(
        builder: (context, state) {
          return Row(
            children: [
              Expanded(
                child: InkWell(
                  onTap: () {
                    saveTemp(state.listAward);
                    context.read<AwardCubit>().addAward();
                  },
                  child: Container(
                    width: double.infinity,
                    padding: const EdgeInsets.all(8),
              ...
              Expanded(
                child: InkWell(
                  onTap: () {
                    saveTemp(state.listAward);
                    context.read<AwardCubit>().save(userResumeId: widget.userResumeId);
                  },
                  child: Container(
                    width: double.infinity,
                    padding: const EdgeInsets.all(8),
  ...
  void saveTemp(List<AwardEntity> listAward) {
    for (var it in listAward) {
      final index = listAward.indexOf(it);
      it.name = _listNameController[index].text;
      it.awardedAt = _listAwardedAtController[index].text;
    }
  }
}
